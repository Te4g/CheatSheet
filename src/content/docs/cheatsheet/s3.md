...
```bash
aws --profile {my_profile} s3api put-bucket-cors --bucket {my_bucket} --cors-configuration '{
    "CORSRules": [
        {
            "AllowedMethods": [
                "GET",
...
```bash
aws --profile {my_profile} s3api get-bucket-cors --bucket {my_bucket}
```


### Generate a presigned URL
```bash
aws s3 presign s3://{bucket_name}/{file_path} --expires-in {duration_in_seconds}
```
